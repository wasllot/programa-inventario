...
class AdminModel extends Query{
    ...
    public function actualizar($rif, $nombre, $telefono, $correo, $direccion, 
    $mensaje, $impuesto, $id)
    {
        $sql = "UPDATE configuracion SET rif=?, nombre=?, telefono=?, correo=?, direccion=?, 
        mensaje=?, impuesto=? WHERE id=?";
    ...
    {
        $sql = "SELECT p.*, c.categoria FROM productos p INNER JOIN categorias c ON p.id_categoria = c.id ORDER BY p.id DESC LIMIT 15";
        return $this->selectAll($sql);
    }
    ...
    {
        $sql = "SELECT descripcion, cantidad FROM productos WHERE cantidad < 5 LIMIT 10";
        return $this->selectAll($sql);
    }
