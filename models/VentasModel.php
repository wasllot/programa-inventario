...
class VentasModel extends Query{
    ...
    public function registrarVenta($productos, $total, $fecha, $metodo, $descuento, $serie, $idCliente, $idusuario)
    {
        $sql = "INSERT INTO ventas (productos, total, fecha, metodo, descuento, serie, id_cliente, id_usuario) VALUES (?,?,?,?,?,?,?,?)";
        $array = array($productos, $total, $fecha, $metodo, $descuento, $serie, $idCliente, $idusuario);
    ...
    public function registrarCredito($monto, $fecha, $idVenta)
    {
        $sql = "INSERT INTO creditos (monto, fecha, id_venta) VALUES (?,?,?)";
        $array = array($monto, $fecha, $idVenta);
        return $this->insertar($sql, $array);
    }
    ...
    {
        $sql = "SELECT v.*, c.identidad, c.num_ci, c.nombre, c.telefono, c.direccion FROM ventas v INNER JOIN clientes c ON v.id_cliente = c.id WHERE v.id = $idVenta";
        return $this->select($sql);
    }
