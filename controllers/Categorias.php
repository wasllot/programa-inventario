...
class Categorias extends Controller
{
    ...
    public function registrar()
    {
        if (isset($_POST['nombre'])) {
            ...
            if (empty($categoria)) {
                ...
                if ($id == '') {
                    $verificar = $this->model->Validar('categoria', $categoria, 'registrar', 0);
                    if (empty($verificar)) {
                    ...
                    $verificar = $this->model->Validar('categoria', $categoria, 'actualizar', $id);
                    if (empty($verificar)) {
    ...
    public function eliminar($idCategoria)
    {
        if (isset($_GET) && is_numeric($idCategoria)) {
            ...
            if ($data == 1) {
                $res = array('msg' => 'CATEGORIA ELIMINADA', 'type' => 'success');
            } else {
                $res = array('msg' => 'ERROR AL ELIMINAR', 'type' => 'error');
            }
        } else {
            $res = array('msg' => 'ERROR DESCONOCIDO', 'type' => 'success');
        }
