...
class Clientes extends Controller
{
    ...
    {
        if (isset($_POST['identidad']) && isset($_POST['num_ci'])) {
            $id = strClean($_POST['id']);
            $identidad = strClean($_POST['identidad']);
            $num_ci = strClean($_POST['num_ci']);
            ...
            if (empty($identidad)) {
            ...
            } else if (empty($num_ci)) {
                $res = array('msg' => 'EL N° DE CÉDULA ES REQUERIDO', 'type' => 'warning');
                ...
                if ($id == '') {
                    $verificarIdentidad = $this->model->getValidar('num_ci', $num_ci, 'registrar', 0);
                    if (empty($verificarIdentidad)) {
                        ...
                        if (empty($verificarTelefono)) {
                            ...
                            $res = array('msg' => 'TELEFONO YA REGISTRADO', 'type' => 'warning');
                        }
                    } else {
                        $res = array('msg' => 'CÉDULA YA REGISTRADA', 'type' => 'warning');
                    }
                } else {
                    $verificarIdentidad = $this->model->getValidar('num_ci', $num_ci, 'actualizar', $id);
                    if (empty($verificarIdentidad)) {
    ...
    public function eliminar($idCliente)
    {
        if (isset($_GET) && is_numeric($idCliente)) {
            ...
            if ($data > 0) {
                $res = array('msg' => 'CLIENTE ELIMINADO', 'type' => 'success');
            } else {
